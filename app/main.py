...
async def run_azure_open_ai(
    ...
    message = {"type": "info", "message": "Process Initiated"}
    ...
    message = {"type": "info", "message": f"Process ID Generated - {nlp_id}"}
    await websocket.send_json(message)
    ...
    for document_id in request.document_list:
        try:
            ...
            message = {
                "type": "info",
                "message": f"Processing {count} of {len(request.document_list)}",
            }
            await websocket.send_json(message)

            message = {
                "type": "info",
                "message": f"Process started for the Document ID - {document_id}",
            }
            await websocket.send_json(message)
            ...
            message = {
                "type": "info",
                "message": f"OCR Text pulled for the Document ID - {document_id}",
            }
            await websocket.send_json(message)
            ...
            message = {
                "type": "success",
                "message": f"Process completed for the Document ID - {document_id}",
            }
            await websocket.send_json(message)
            document_count += 1
            ...
            message = {
                "type": "error",
                "message": f"Error while procesing Document ID = {document_id}, ERROR: {str(ex)}",
            }
            await websocket.send_json(message)

    try:
        ground_truth_list = Database.get_ground_truth_by_nlp_id(session, nlp_id)
        Database.insert_nlp_accuracy(session, nlp_id, document_count, ground_truth_list)
        message = {
            "type": "info",
        ...
        message = {
            "type": "error",
            "message": f"Error while generating NLP Accuracy, ERROR: {str(ex)}",
        }
        await websocket.send_json(message)

    message = {"type": "success", "message": "COMPLETED"}
    await websocket.send_json(message)
