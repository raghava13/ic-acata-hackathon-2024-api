...
from sqlmodel import Boolean, Column, DateTime, Field, Integer, Relationship, String
...
class Document(CamelSQLModel, table=True):
    ...
    combined_division_mpi: str = Field(sa_column=Column("CombinedDivisionMpi", String))
    document_name: str = Field(sa_column=Column("DocumentName", String))
    document_type_description: str = Field(
        sa_column=Column("DocumentTypeDescription", String)
    )
